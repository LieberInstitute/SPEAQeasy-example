...
The following analysis explores a `RangedSummarizedExperiment` object from the SPEAQeasy pipeline. Note that we will use a modified version of the object, which resolved sample identity issues which were present in the raw output from SPEAQeasy. Though SPEAQeasy produces objects for several feature types (genes, exons, exon-exon junctions), we will demonstrate an example analysis for just genes. We will first add phenotype data we have for the samples to the `RangedSummarizedExperiment`, and then perform differential expression across some typical variables of interest (e.g. sex, age, race). This guide also shows how to perform principal component analysis (PCA) and visualize findings with plots.
...
```{r loaddata}
# Set this path appropriately for your particular set-up
setwd('/dcl01/lieber/ajaffe/lab/SPEAQeasy-example/')

# Load the RSE gene object, and phenotype data for the samples
load('pipeline_outputs/count_objects/rse_gene_Jlab_experiment_n42.Rdata')
load('sample_selection/pd_example.Rdata')

# SPEAQeasy includes a custom 'getRPKM' function with the RSE objects, but this
# will not be used for the upcoming analysis
rm(getRPKM)
...
## Attach phenotype data to the RSE object

Adding phenotype data to the RSE object enables us to perform differential expression across variables of interest. We add this information to the `colData` of the RSE object.
